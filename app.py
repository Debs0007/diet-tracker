...
# Load creds from Streamlit secrets (Streamlit Cloud) or run locally with credentials.json
creds_dict = st.secrets["gcp_service_account"]
creds = ServiceAccountCredentials.from_json_keyfile_dict(creds_dict, scope)
client = gspread.authorize(creds)
spreadsheet = client.open(SHEET_NAME)

# Helper to ensure worksheets exist (create if missing)
...
# Ensure needed sheets exist with headers
...
with col2:
    ...
    # Determine current month label (e.g., "2023-06")
    today = datetime.date.today()
    month_year_label = st.selectbox("Select month (change to create/update goal)",
...
def upsert_goal(ws, month_label, cal_goal, prot_goal):
    ...
    for i, r in enumerate(records, start=2):  # gspread is 1-indexed, header is row 1
        if r.get("month_year") == month_label:
            ws.update(f"B{i}", cal_goal)  # B column -> calorie_goal
            ws.update(f"C{i}", prot_goal) # C column -> protein_goal
...
def append_meal_row(ws, row):
    # row = [date, time, food, qty_g, kcal, protein, carbs, fat, fiber, notes]
    ws.append_row(row)
...
if add_btn:
    if not food_item:
        ...
        try:
            ...
            # clear local UI values by doing nothing - user can change them
...
# Load meals for the day from sheet
